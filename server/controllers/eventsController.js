...
export const getEventsByLocationSlug = async (req, res) => {
  try {
    ...
    const locationResult = await pool.query('SELECT * FROM locations WHERE LOWER(REPLACE(name, \' \', \'\')) = $1', [locationSlug]);
    ...
    const eventResult = await pool.query('SELECT * FROM events WHERE location_id = $1', [location.id]);
